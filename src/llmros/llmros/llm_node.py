...
You must only output JSON. Do not include markdown code blocks or any extra text.
...
{{
    ...
    "params": {{
        "name": "a string for the location's name"
    }}
}},
{{
...
prompt: "여기를 내 책상으로 저장해줘"
returns: {{"action": "save_location", "params": {{"name": "내 책상"}}}}

prompt: "화장실로 가자"
returns: {{"action": "go_to_location", "params": {{"name": "화장실"}}}}

prompt: "내 책상 지워줘"
returns: {{"action": "delete_location", "params": {{"name": "내 책상"}}}}

prompt: "저장된 장소 목록 보여줘"
returns: {{"action": "list_locations", "params": {{}}}}
...
'''
        ...
        try:
            ...
            try:
                ...
                return json.dumps({"action": "error", "message": "LLM response parsing failed"})

        except Exception as e:
            self.get_logger().error(f'[GPT Error] {str(e)}')
            return json.dumps({"action": "error", "message": f"GPT API error: {str(e)}"})
